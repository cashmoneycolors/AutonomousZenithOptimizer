...
class MiningControlPanel:
    ...
    def create_gui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="CASH MONEY COLORS ORIGINAL (R) MINING CONTROL PANEL",
            font=("Arial", 16, "bold"),
            foreground="#FFD700",
            background="#1a1a1a",
        )
        ...
        self.optimize_btn = ttk.Button(
            button_frame, text="FORCE OPTIMIZE", command=self.force_optimize
        )
        self.optimize_btn.pack(side=tk.LEFT, padx=5)

        self.reset_btn = ttk.Button(
            button_frame, text="RESET SYSTEM", command=self.reset_system
        )
        ...
        ttk.Label(status_grid, text="Status:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky=tk.W, padx=5
        )
        self.status_label = ttk.Label(
            status_grid, text="STOPPED", foreground="red", font=("Arial", 10, "bold")
        )
        self.status_label.grid(row=0, column=1, sticky=tk.W, padx=5)

        ttk.Label(status_grid, text="Capital:", font=("Arial", 10, "bold")).grid(
            row=1, column=0, sticky=tk.W, padx=5
        )
        self.capital_label = ttk.Label(
            status_grid,
            text="100.00 CHF",
            foreground="#FFD700",
            font=("Arial", 12, "bold"),
        ...
        ttk.Label(status_grid, text="Total Profit:", font=("Arial", 10, "bold")).grid(
            row=2, column=0, sticky=tk.W, padx=5
        )
        self.profit_label = ttk.Label(
            status_grid, text="0.00 CHF", foreground="green", font=("Arial", 12, "bold")
        )
        self.profit_label.grid(row=2, column=1, sticky=tk.W, padx=5)

        ttk.Label(status_grid, text="Active Rigs:", font=("Arial", 10, "bold")).grid(
            row=3, column=0, sticky=tk.W, padx=5
        )
        self.rigs_label = ttk.Label(
            status_grid, text="6", foreground="blue", font=("Arial", 12, "bold")
        )
        self.rigs_label.grid(row=3, column=1, sticky=tk.W, padx=5)

        ttk.Label(status_grid, text="Cycles:", font=("Arial", 10, "bold")).grid(
            row=4, column=0, sticky=tk.W, padx=5
        )
        self.cycles_label = ttk.Label(
            status_grid, text="0", foreground="purple", font=("Arial", 12, "bold")
        )
        ...
        self.rigs_tree = ttk.Treeview(
            rigs_frame, columns=columns, show="headings", height=8
        )
        ...
        for col in columns:
            ...
            scrollbar = ttk.Scrollbar(
                rigs_frame, orient=tk.VERTICAL, command=self.rigs_tree.yview
            )
        ...
        # Initial Log
        self.log_message(
            "CASH MONEY COLORS ORIGINAL (R) MINING CONTROL PANEL INITIALIZED"
        )
    ...
    def reset_system(self):
        ...
        if messagebox.askyesno(
            "Reset System", "Wirklich das gesamte System zurücksetzen?"
        ):
            self.stop_mining()
            self.capital = 100.0
            self.total_profit = 0.0
    ...
    def mining_loop(self):
        ...
        while self.is_running:
            ...
            self.log_message(
                f"Cycle {self.cycles_completed}: +{cycle_profit:.2f} CHF (Total: {self.capital:.2f} CHF)"
            )
    ...
    def calculate_mining_profit(self):
        ...
        for rig in self.active_rigs:
            if rig["status"] == "ACTIVE":
                ...
                market_factor = 0.9 + (
                    0.2 * (1 + 0.1 * (0.5 - time.time() % 1))
                )  # Pseudo-random market
                efficiency = 0.95 + (
                    0.1 * (0.5 - time.time() % 1)
                )  # Hardware efficiency
                ...
                for algo, coin in algorithms[rig_type]:
                    if algo != current_algo:
                        ...
                        if new_profit > rig["profit_per_day"] * 1.05:  # 5% better
                            ...
                            self.log_message(
                                f"OPTIMIZATION: {rig['id']} {old_coin}({current_algo}) -> {coin}({algo})"
                            )
                            break
