...
- `Core/`
  - `Core/Interfaces.cs`: **stabile Contracts** fÃ¼r Adapter/Module (`IProfitGuarantor_QML`, `IHyperCache`, `IHFT_AMAD_Adapter`, ...).
...
- `Modules/`
  ...
  - `Modules/HoloCache.cs`: Cache/Redis-Abstraktion, Key: `context:{query}`, TTL aktuell 5 Minuten; enthÃ¤lt `ContextualMemoryHandler`.
...
- `Adapters/`: Side-Effects/Integrationsgrenzen (HFT/CRM/KI). Ã„nderungen hier sind die AuÃŸenkante des Systems.
...
## Ã„nderungsregeln (repo-spezifisch, wichtig)
- `Core/Interfaces.cs` nicht heimlich brechen: bei SignaturÃ¤nderungen immer Implementierungen + Tests in einem Zug anpassen.
- Neue Integrationen immer hinter ein Interface legen und in `Program.cs` per DI registrieren (kein verstreutes `new`).
- Retry/Fallback der QML-Integration nur Ã¼ber `ExecuteQMLWithRetry()` erweitern (Single Source of Truth).
